...
const ProfileTab = () => {
  ...
  return (
    ...
    >
        ...
        <View className="items-center mt-10">
          <View className="relative">
            ...
            <Pressable className="absolute bottom-1 right-1 w-8 h-8 bg-primary rounded-full items-center justify-center border-2 border-surface-dark">
              <Ionicons name="camera-outline" size={16} color="#0d0d0f" />
            </Pressable>
          </View>
      ...
      {/* Sign Out Button */}
      <Pressable
        className="mx-5 mt-8 bg-red-500/20 px-3 rounded-full py-4 items-center active:opacity-70"
        onPress={() => signOut()}
      >
        <View className="flex-row items-center">
          <Ionicons name="log-out-outline" size={20} color="#EF4444" />
          <Text className="text-red-500 text-sm font-semibold ml-2">Log Out</Text>
        </View>
      </Pressable>
    </ScrollView>
  );
